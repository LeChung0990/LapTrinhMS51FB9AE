...
/**
 * @brief Delay Timer0 Init
 * @return  None
 * @note Timer0 Clock = Fsys/12 = 16MHz/12 = 1.3Mhz
 * Timer0 is 16-bit mode
 */
void DelayT0_Init(void)
{
  TIMER0_FSYS_DIV12;   // T0M=0, Timer0 Clock = Fsys/12 = 16MHz/12 = 1.3Mhz
  ENABLE_TIMER0_MODE1; // Timer0 is 16-bit mode
}

/**
 * @brief Timer0 delay setting
 * @param[in] count define count time.
 ...
 * @example :  Timer0_Delay(1,1000);
 */
void T0_Delay(uint16_t u16DLYUnit)
{
  uint8_t TL0TMP, TH0TMP;
  TL0TMP = LOBYTE(65535 - ((16000000 / 1000000) * u16DLYUnit / 12));
  TH0TMP = HIBYTE(65535 - ((16000000 / 1000000) * u16DLYUnit / 12));
